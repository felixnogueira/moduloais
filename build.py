...
hbb_name = 'moduloais' + ('.exe' if windows else '')
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/etc/moduloais/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/moduloais/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/moduloais || true')
    system2(
    ...
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/moduloais/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/moduloais/')
    system2(
        'cp ../res/pam.d/moduloais.debian tmpdeb/etc/pam.d/moduloais')
    system2(
    ...
    os.rename('moduloais.deb', '../moduloais-%s.deb' % version)
    os.chdir("..")
...
def build_flutter_dmg(version, features):
    ...
    '''
    system2(
        "create-dmg --volname \"Modulo AIS Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon ModuloAis.app 200 190 --hide-extension ModuloAis.app moduloais.dmg ./build/macos/Build/Products/Release/ModuloAis.app")
    os.rename("moduloais.dmg", f"../moduloais-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    if os.path.exists('./moduloais_portable.exe'):
        ...
        os.rename('./target/release/moduloais-portable-packer.exe',
                  './moduloais_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/moduloais_portable.exe')
    os.rename('./moduloais_portable.exe', f'./moduloais-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/moduloais-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/moduloais/moduloais.exe')
        system2('mv target/release/moduloais.exe target/release/moduloais.exe')
        pa = os.environ.get('P')
        if pa:
            ...
            print('Not signed')
        system2(
            f'cp -rf target/release/moduloais.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/moduloais-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/moduloais-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/moduloais')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv moduloais-%s-0-x86_64.pkg.tar.zst moduloais-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./moduloais.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/moduloais-%s-0.x86_64.rpm ./moduloais-%s-fedora28-centos8.rpm' % (
                version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/moduloais-%s-0.x86_64.rpm ./moduloais-%s-suse.rpm' % (
                version, version))
        # yum localinstall rustdesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/moduloais*.deb ./flutter/moduloais.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/ModuloAis.app/Contents/MacOS/moduloais')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/ModuloAis.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/moduloais-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/ModuloAis.app/Contents/MacOS/moduloais
    #rcodesign sign --p12-file ~/.p12/moduloais-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/ModuloAis.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/moduloais-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/ModuloAis.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/ModuloAis.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/ModuloAis.app
    '''.format(pa))
                system2(
                    'create-dmg "ModuloAis %s.dmg" "target/release/bundle/osx/ModuloAis.app"' % version)
                os.rename('ModuloAis %s.dmg' %
                          version, 'moduloais-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/moduloais-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./moduloais-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./moduloais-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple moduloais-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/ModuloAis.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/moduloais*.deb ./moduloais.deb')
                system2('dpkg-deb -R moduloais.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/moduloais/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                os.system('cp res/xorg.conf tmpdeb/etc/X11/moduloais/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/moduloais.debian tmpdeb/etc/pam.d/moduloais')
                system2('strip tmpdeb/usr/bin/moduloais')
                system2('mkdir -p tmpdeb/usr/lib/moduloais')
                ...
                md5_file('usr/lib/moduloais/libsciter-gtk.so')
                system2('dpkg-deb -b tmpdeb moduloais.deb; /bin/rm -rf tmpdeb/')
                os.rename('moduloais.deb', 'moduloais-%s.deb' % version)
