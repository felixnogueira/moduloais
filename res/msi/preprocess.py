...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="ModuloAIS", help="The app name."
    )
    parser.add_argument(
...
def gen_auto_component(app_name, dist_dir):
    return gen_content_between_tags(
        "Package/Components/ModuloAIS.wxs",
        "<!--$AutoComonentStart$-->",
        "<!--$AutoComponentEnd$-->",
...
def update_license_file(app_name):
    if app_name == "ModuloAIS":
        return
    license_file = Path(sys.argv[0]).parent.joinpath("Package/License.rtf")
    with open(license_file, "r") as f:
        license_content = f.read()
    license_content = license_content.replace("website aislabs.io and other ", "")
    license_content = license_content.replace("ModuloAIS", app_name)
