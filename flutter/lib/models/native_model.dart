...
class PlatformFFI {
  ...
  ModuloaisImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(ModuloaisImpl moduloaisImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = moduloaisImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
